let 国民任务 = async (是否需要初始化 = true) => require('../api')().then(async () => {
    ...
    let 启动开始地图 = ['召唤之间', '回廊', '灵堂', '谒见之间', '里谢里雅堡 2楼'];
    if (!启动开始地图.find(n => 当前地图 == n)) {
        信息提示('脚本提前结束，从不支持的地图启动任务，请尝试回到出生地 召唤之间 启动脚本');
        return;
    }
